...
public class SwerveSubsytem extends SubsystemBase {
    ...
    private SwerveDriveKinematics swerve_kinematics = new SwerveDriveKinematics(galit_location,iris_location,idan_location,amalia_location);
    private ChassisSpeeds swerve_speeds;
    private NavxGyro gyro = new NavxGyro(Port.kMXP);
    private SwerveDriveOdometry swerve_odemetry = new SwerveDriveOdometry(swerve_kinematics,gyro.getHeading(),new Pose2d(0,0,new Rotation2d())); // update start position
    ...
    {
        this.current_pose = swerve_odemetry.update(gyro.getHeading(),galit.module_state,iris.module_state,idan.module_state,amalia.module_state);
    }
    public void set_modules_field_orianted(double x_velocity, double y_velocity, double rotation_velocity)
    {
        ...
        this.swerve_speeds = ChassisSpeeds.fromFieldRelativeSpeeds(x_velocity,y_velocity, rotation_velocity, gyro.getHeading());
        SwerveModuleState[] target_states = this.swerve_kinematics.toSwerveModuleStates(this.swerve_speeds);
        galit.set_module_state(target_states[0]);
        iris.set_module_state(target_states[1]);
        idan.set_module_state(target_states[2]);
